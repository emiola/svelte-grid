...
<div class:svlt-grid-transition={!focusedItem} class="svlt-grid-container" bind:this={container} style="height: {ch}px">
  {#each items as item, i (item.id)}
    <div
      on:mousedown={item.draggable ? dragOnMouseDown.bind(this, item.id) : null}
      ...
      width: {item.resize.resizing ? item.resize.width : item.w * xPerPx - gap * 2 - item.responsive.valueW * xPerPx}px; height: {item.resize.resizing ? item.resize.height : item.h * yPerPx - gap * 2}px; z-index: {item.drag.dragging || item.resize.resizing ? items.length + 2 : item.id + 1}; opacity:
      {item.resize.resizing ? 0.5 : 1}"
      item-id={item.id}>
    ...
    {#if shadow.active}
      <div
        ...
        {!useTransform ? `left: ${shadow.drag.dragging ? shadow.drag.left : shadow.x * xPerPx + gap}px` : ''}; width:{shadow.w * xPerPx - gap * 2 - shadow.responsive.valueW * xPerPx}px; height: {item.collapsed ? rowHeight : shadow.h * yPerPx - gap * 2}px; z-index: 0; opacity: 1;" />
    {/if}
  {/each}
</div>
